...
describe("Fuzz Testing: Schema Validation", () => {
  ...
  describe("SerializedRegexSchema", () => {
    ...
    it("rejects patterns detected as unsafe by redos-detector", () => {
      ...
      const unsafePatterns = ["(a+)+", "(.*)*", "([a-z]+)*([a-z]+)*"];
...
describe("Security Property Tests", () => {
          ...
          (testId, pathSegments, value) => {
            ...
            for (const newKey of protoKeysAfter) {
              if (!protoKeysBefore.has(newKey)) {
                throw new Error(
                  `Prototype pollution detected: new key "${newKey}" in Object.prototype`,
                );
              }
            }
      ...
      fc.assert(
        fc.property(
          ...
          (dangerousKey, value) => {
            ...
            expect(
              ({} as Record<string, unknown>)[POLLUTION_MARKER],
            ).toBeUndefined();
      ...
      fc.assert(
        fc.property(
          ...
          (prefix, dangerous, value) => {
            ...
            expect(
              stateManager.get("test", `${prefix}.${dangerous}`),
            ).toBeUndefined();
  ...
  describe("extractFromPath: Prototype Pollution Prevention", () => {
      ...
      fc.assert(
        fc.property(
          ...
          (safeKey, dangerousKey, value) => {
            ...
            const result = extractFromPath(
              context,
              `body.${safeKey}.${dangerousKey}`,
            );
    ...
    it("PROPERTY: Inherited properties are never accessible", () => {
      ...
      fc.assert(
        fc.property(fc.constantFrom(...inheritedProps), (inheritedProp) => {
          const context = createContext({});
    ...
    it("PROPERTY: Only own properties are accessible", () => {
      ...
      fc.assert(
        fc.property(safeKeyArb, fc.anything(), (key, value) => {
    ...
    it("PROPERTY: Malicious template-like patterns complete quickly", () => {
      ...
      const maliciousPatterns = fc.oneof(
        ...
        fc
          ...
          .map((n) => "{".repeat(n) + "state.key" + "}".repeat(n)),
        // Nested template-like patterns
        fc
          .integer({ min: 1, max: 50 })
          .map((n) => "{{state." + "|".repeat(n) + "}}"),
        // Very long paths
        fc
          .integer({ min: 1, max: 300 })
          .map((n) => "{{state." + "a".repeat(n) + "}}"),
        // Mixed valid and invalid
        fc
          .array(fc.constantFrom("{{state.key}}", "{{", "}}", "{", "}"), {
            minLength: 1,
            maxLength: 100,
          })
      ...
      fc.assert(
        fc.property(
          ...
          (key, value) => {
            ...
            expect(Object.keys(Object.prototype).length).toBeLessThanOrEqual(
              20,
            ); // Reasonable limit
