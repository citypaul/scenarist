    ...
    const capturedParams = await new Promise<MSWParams>((resolve) => {
      server.use(
        http.get(mswPattern, ({ params, request }) => {
          console.log(
            `[MSW HANDLER] pattern=${mswPattern}, url=${request.url}, params=`,
            params
          );
          resolve(params as MSWParams);
...
describe("MSW Parity: Path Parameter Extraction", () => {
  ...
  describe("Optional Parameters (:param?)", () => {
    ...
    it("should handle multiple optional params - all present", async () => {
      await testParamParity("/posts/:year?/:month?/:day?", "/posts/2024/11/17");
    });
  ...
  describe("Repeating Parameters (:param+, :param*)", () => {
    ...
    it("should handle :path+ with special characters", async () => {
      await testParamParity("/files/:path+", "/files/@types/node/index.d.ts");
    });
  });
  ...
  describe("URL Encoding", () => {
    ...
    it("should handle deeply nested paths", async () => {
      await testParamParity("/a/:b/c/:d/e/:f/g/:h", "/a/1/c/2/e/3/g/4");
    });
