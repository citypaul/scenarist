...
export const setupUrlMatchingRoutes = (router: Router): void => {
  ...
  router.get('/api/test-url-match/numeric-id/:userId', async (req: Request, res: Response) => {
  ...
  router.get('/api/test-url-match/contains-api/:city', async (req: Request, res: Response) => {
    return handleProxyRequest(
      (req) => ({
  ...
  router.get('/api/test-url-match/version/:version/:city', async (req: Request, res: Response) => {
    return handleProxyRequest(
      (req) => ({
  ...
  router.get('/api/test-url-match/file-extension/:filename', async (req: Request, res: Response) => {
    return handleProxyRequest(
      (req) => ({
  ...
  router.get('/api/test-url-match/combined', async (req: Request, res: Response) => {
    return handleProxyRequest(
      (req) => ({
        url: 'https://api.stripe.com/v1/charges',
        headers: buildHeaders({ 'x-api-version': req.query.apiVersion as string }),
      }),
      req,
      res
    );
  ...
  router.get('/api/test-url-match/exact/:username', async (req: Request, res: Response) => {
    return handleProxyRequest(
      (req) => ({
