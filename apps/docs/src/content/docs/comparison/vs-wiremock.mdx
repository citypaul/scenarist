...
| **Language** | TypeScript/JavaScript | Language-agnostic (Node.js via wiremock-captain) |
| **Test isolation** | Per-test via header | Per-server instance or Scenarios |
| **Runtime switching** | Yes (single API call) | Yes (Admin API) |
| **Setup** | npm install | Docker + network configuration |
...
**Trade-off:** Scenarist's in-process approach means simpler setup—just npm install and configure. No Docker, no network proxies, no environment variables to manage. WireMock's server approach works with any language but requires Docker, network configuration, and proxy setup to route your app's requests to WireMock.
...
<CardGrid>
  <Card title="Simple setup" icon="rocket">
    Just npm install and configure scenarios. No Docker, no network proxies, no environment variables to manage.
  </Card>
...
|--------|-----------|----------|
| Setup simplicity | ✓ npm install | Docker + network config |
| TypeScript integration | ✓ Native | Via wiremock-captain |
...
**Bottom line:** Choose Scenarist for TypeScript/JavaScript projects where you value simple setup (no Docker/network config), header-based parallel test isolation, and first-class Playwright support. Choose WireMock for polyglot environments, when you need recording, contract testing, or when you're already invested in the WireMock ecosystem. Both tools are capable—the choice is primarily about simplicity vs. language flexibility.
