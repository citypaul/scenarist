...
<CardGrid>
  ...
  <Card title="Response Sequences" icon="list-format">
    Built-in support for polling APIs, retry flows, and state machines. <a href="/concepts/dynamic-responses">Return different responses</a> on successive calls.
  </Card>
  <Card title="Developer Tools (Roadmap)" icon="puzzle">
    <a href="/roadmap">Planned browser-based plugin</a> for switching scenarios during development—instant visual scenario exploration without code changes.
  </Card>
</CardGrid>
...
| **Setup complexity** | npm install | Docker + network config | npm install | Docker required | Built-in |
| **Parallel tests** | Built-in (header routing) | Multiple instances or scenarios | Manual isolation | Per-container | Per-page |
