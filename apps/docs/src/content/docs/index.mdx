---
title: Scenarist
description: Browser-based testing where your backend actually runs. Test Server Components, API routes, and validation logic with real execution.
template: splash
hero:
  title: Browser Testing That Actually Runs Your Backend
  tagline: Server Components can't be unit tested—you need browser-based tests. But traditional approaches either mock everything (your code never runs) or require slow app restarts per scenario. Scenarist mocks only external APIs while your Server Components render, API routes execute, and validation logic runs. Instant scenario switching in Playwright with type-safe helpers.
  image:
...
**Why this matters for Server Components:** Next.js Server Components can't be unit tested—they require browser-based testing. But traditional mocking means they never execute. Scenarist lets you test complete user journeys **while your Server Components actually render.**
:::
...
**Playwright (Mocked)**: Backend never runs—you're testing mock responses, not your code
...
**Scenarist + Playwright**: Real backend execution + mocked external APIs + instant scenario switching + built-in state = fast full-stack tests
:::
