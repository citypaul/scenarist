...
| **Multiple Scenarios** | ✅ Unlimited (with mocks) | ❌ 1-2 only (too slow) | ✅ **Unlimited, fast** |
| **Test Speed** | ✅ Very fast (50ms) | ❌ Slow (5-10s per scenario) | ✅ **Fast (2s per scenario)** |
...
- Testing 10 scenarios = 10 separate test runs (slow)
- Can't test error states without hitting real APIs
- Too slow for comprehensive coverage
...
// 10 scenarios = 20 seconds total (parallel)
```
:::
