...
│                                                                  │
│  ┌──────────────────┐                                           │
│  │   🌐 Browser     │  ← Real UI, real user interactions        │
│  │   (Playwright)   │                                           │
│  └────────┬─────────┘                                           │
│           │                                                      │
...
│           ↓                                                      │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │   ⚙️  YOUR BACKEND LOGIC (Runs Normally)                  │  │
│  │                                                            │  │
│  │   • Next.js Server Components                             │  │
│  │   • Express route handlers                                │  │
│  │   • Business logic, validation, formatting                │  │
│  │   • Database queries (real or test DB)                    │  │
│  │                                                            │  │
│  │   ✅ Your code executes exactly as it would in production │  │
│  └────────┬───────────────────────────────────────────────────┘  │
│           │                                                      │
│           │ Needs external data: POST https://api.stripe.com/... │
│           ↓                                                      │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │   🎭 SCENARIST MOCK LAYER (MSW)                           │  │
│  │                                                            │  │
│  │   • Intercepts external API calls                         │  │
│  │   • Returns test data from scenarios                      │  │
│  │   • Stripe ✓  Payment gateways ✓  Third-party APIs ✓     │  │
│  │                                                            │  │
│  │   ⚠️  Ephemeral endpoints - only exist during tests       │  │
│  └──────────────────────────────────────────────────────────┘  │
│           │                                                      │
│           │ ❌ Real Stripe API never called                      │
│           ↓                                                      │
│  ┌──────────────────┐                                           │
│  │ 🚫 External APIs │  ← Never reached in tests                 │
│  │ (Stripe, etc.)   │                                           │
│  └──────────────────┘                                           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
- ⚠️ Stripe payment processing → mocked response
- ⚠️ Email services → mocked confirmation
- ⚠️ Third-party APIs → mocked data
