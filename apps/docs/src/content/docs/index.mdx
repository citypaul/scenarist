...
Unit tests work great, but they test code in isolation—not how it actually runs in production:
- ❌ No real server-side context (sessions, auth state, request lifecycle)
- ❌ Server Components are especially hard to unit test properly
- ❌ Functions tested individually, not as integrated user journeys
- ❌ Missing the entry points and middleware that shape real execution
...
**The reality:** Your server-side code executes as part of user journeys with real sessions, auth context, and middleware—but you're testing it either in isolation (unit tests) or only for the happy path (browser tests).
...
| **Server-Side Context** | ❌ Mock sessions/auth | ❌ Backend mocked away | ✅ **Real sessions, real context** |
| **Middleware Chains** | ⚠️ Can test, but isolated | ❌ Never execute | ✅ **Real execution as integrated flow** |
| **Validation Logic** | ✅ Individual validators | ❌ Not tested with HTTP layer | ✅ **Through full request flow** |
| **Server Components** | ⚠️ Hard to test properly | ❌ Not tested | ✅ **Real rendering with server context** |
| **Error Scenarios** | ✅ Unit level only | ❌ Only happy path | ✅ **All scenarios in browser** |
| **User Journey** | ❌ No browser | ✅ Happy path only | ✅ **All scenarios as real journeys** |
| **External APIs** | N/A | Manual per-test setup | ✅ **Scenario-based (MSW)** |
| **Multiple Scenarios** | ✅ Unlimited | ❌ Just 1-2 (happy paths) | ✅ **Unlimited in browser** |
| **Test Speed** | ✅ Very fast | ⚠️ Moderate | ✅ **Fast** |

:::tip[The Gap Scenarist Fills]

**What teams do today:**
- Unit tests → Test functions in isolation (works well, but no real server context or user journeys)
- Browser tests → Test happy path only (real execution, but limited scenarios)

**The gap:** Can't test your backend logic through real HTTP requests with multiple scenarios in the browser. Your code runs differently in unit tests (isolated, no server context) than in production (integrated, with sessions/auth/middleware).
...
**How Scenarist fills it:** Test all scenarios (error cases, edge cases, different user states) through real HTTP requests with real server-side execution (sessions, middleware, Server Components) while mocking only external APIs.
:::
