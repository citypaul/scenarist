...
hero:
  ...
  tagline: Your Server Components render. Your middleware executes. Your validation runs—for real. Only external APIs are mocked. Switch scenarios instantly without restarting your server.
  image:
    file: ../../assets/hero-illustration.svg
    alt: Server code executing with controlled external API scenarios
  actions:
    - text: Get Started
...
:::tip[The Key Insight]
**Stop mocking framework internals.** Test your actual backend code with real HTTP requests. Only external APIs are mocked—and you switch scenarios at runtime without restarting.
...
**What's mocked:**
- Only external APIs (Stripe, SendGrid, Auth0) via MSW
...
- Works everywhere (Express, Next.js, Remix, Fastify, Hono, any Node.js framework)
...
- **Test isolation:** Cryptographic UUIDs per test for perfect isolation
