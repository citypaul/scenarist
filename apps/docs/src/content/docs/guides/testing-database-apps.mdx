...
graph TD
    ...
    G -->|Yes| H[✅ Use API Route Abstraction + Scenarist]
...
**What it is:** Add thin HTTP layer between Server Components and database, mock everything via Scenarist.

### Use When:

- ✅ You want to test **different backend states** (success, errors, edge cases)
- ✅ You want **fast tests** without database setup (instant scenario switching)
...
## Approach 2: Hybrid (Testcontainers + Scenarist)

**What it is:** Use [Testcontainers](https://testcontainers.com/) to spin up real database containers with seeded scenarios. Use Scenarist to mock external API dependencies. No code changes needed.
...
test('premium user with successful payment', async ({ page, switchScenario }) => {
  // Testcontainer seeded with premium user (tier='premium', cartTotal=500)
  const container = await new PostgreSqlContainer().start();
  await seedPremiumUser(container);

  // Scenarist mocks Stripe success
  await switchScenario(page, 'stripeSuccess');

  await page.goto('/checkout');

  // Test sees:
  // - Premium user from real database
...
- ⚠️ **Slower tests** - Container startup takes 5-30 seconds
- ⚠️ **Docker required** - CI/CD must support Docker
- ⚠️ **Database seeding scripts** - Must maintain seed data
- ⚠️ **Harder to parallelize** - Shared database state between tests

**Learn more:** [Testcontainers Documentation](https://testcontainers.com/)
