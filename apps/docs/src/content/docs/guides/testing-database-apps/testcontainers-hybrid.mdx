...
If sequential execution is a blocker for your team, see [Parallelism Options](./parallelism-options) for a detailed comparison of alternatives (multiple containers, test ID columns, PostgreSQL RLS, schema-per-test) and their trade-offs.
</Aside>
