...
For most teams that want scalable, parallel testing of real server-side code through browser-based tests, the **repository pattern** is the best approach:

- Provides same test ID isolation model as Scenarist
- True test-level parallelism (not just shard-level)
- Architectural benefits extend beyond testing (clean separation, infrastructure flexibility)
- Test repository implementations with Testcontainers for real database behavior

[Learn about the Repository Pattern →](./repository-pattern)

### Other Options

**Sequential Execution** - Simplest but doesn't scale
- Run database tests sequentially, HTTP tests in parallel
- No code changes required
- CI time grows linearly with test count—only suitable for small test suites or prototypes

**Sharding** - Parallelism without code changes
- One database container per Playwright shard
- Tests within each shard still run sequentially
- Requires good CI/CD infrastructure

**Testcontainers** - For testing real database behavior
- Tests actual SQL queries, constraints, transactions
- Best used alongside repository pattern to validate implementations
- Sequential test execution when used standalone
