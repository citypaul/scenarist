...
Scenarist isolates HTTP tests via test ID in headers. Each test gets a unique ID, and MSW uses that ID to return the correct mock responses.

**Databases have no equivalent mechanism.** There's no "test ID header" in database protocols—so parallel tests corrupt each other's data.

## The Solution

The [repository pattern](https://martinfowler.com/eaaCatalog/repository.html) creates that mechanism at the application level. By abstracting database access behind interfaces, you can inject test implementations that partition data by test ID—giving you the same isolation model as Scenarist's HTTP mocking.

Beyond testing, the repository pattern is a well-established architectural practice that provides [significant additional benefits](#beyond-testing-why-the-repository-pattern-is-good-practice) including infrastructure flexibility, better separation of concerns, and alignment with SOLID principles.
...
Complement in-memory tests with a smaller suite of integration tests against a real database using [Testcontainers](./testcontainers-hybrid). Run the fast in-memory tests in parallel for most coverage, then run sequential database tests for critical paths.

</Aside>

### Query Logic Duplication

Complex queries must be reimplemented in the test repository:

```typescript
// Production: ORM handles the query
...
// Test: You reimplement the logic
async search(query: string) {
  const lowerQuery = query.toLowerCase();
  return Array.from(this.getTestStore().values()).filter(
    user =>
