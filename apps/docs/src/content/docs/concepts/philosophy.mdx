...
Scenarist deliberately prevents you from writing functions in scenarios. This isn't a limitation—it's a feature that mirrors successful patterns across software:
...
When you're tempted to write `if (req.something)`, the constraint forces you to ask: "What pattern am I actually trying to express?"
...
Scenarist's testing philosophy is built on these core principles:
...
3. **Constraints guide design.** The inability to write functions forces better abstractions—match criteria, sequences, and state patterns.

4. **Fill the testing gap.** Use Scenarist for HTTP-layer testing that's too slow for E2E but too integrated for unit tests.

5. **Architecture matters.** Testability comes from good design. Clear boundaries make testing natural.
...
- [Express Getting Started](/frameworks/express/getting-started) — Complete Express setup with Supertest
- [Next.js App Router](/frameworks/nextjs-app-router/getting-started) — Server Components, Route Handlers
- [Next.js Pages Router](/frameworks/nextjs-pages-router/getting-started) — API routes, getServerSideProps
