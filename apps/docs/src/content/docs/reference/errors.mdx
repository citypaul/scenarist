...
| `SEQUENCE_EXHAUSTED` | Sequence with `repeat: 'none'` has no more responses | Request to exhausted sequence |
| `NO_STATE_MATCH` | stateResponse has no matching condition and no default | Request with unmatched state |
...
const scenarist = createScenarist({
  ...
  errorBehaviors: {
    ...
    onNoStateMatch: 'ignore',     // Silently continue
