...
**Why this matters:** Scenarist handles a Next.js-specific issue for you. Next.js's webpack bundler can duplicate modules across multiple chunks, causing the same file to be loaded multiple times in the same process ([see Next.js Discussion #68572](https://github.com/vercel/next.js/discussions/68572)). This breaks the singleton pattern and can cause:
...
**The pattern:** Always use `export const scenarist = createScenarist(...)`. Scenarist handles the complex singleton logic for you so you don't need to understand webpack internals or manage global variables yourself.

:::note[Why Scenarist Handles This For You]
