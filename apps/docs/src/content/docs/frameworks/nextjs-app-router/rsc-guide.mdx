...
React Server Components run **exclusively on the server** as async functions. This creates a fundamental incompatibility with traditional React testing tools:
...
Scenarist + Playwright solves these problems by testing Server Components **as they actually run** - in a real Next.js environment with actual server-side rendering:

```typescript
// This WORKS with Scenarist + Playwright:
test('renders premium products', async ({ page, switchScenario }) => {
  await switchScenario(page, 'premiumUser');

  await page.goto('/products?tier=premium');

  // Server Component executes, fetches from mocked API, renders HTML
...
- **Real execution** - Your Server Component code actually runs
- **Real rendering** - HTML is generated server-side, just like production
- **No mocking Next.js** - No stubbing `headers()`, `cookies()`, or other internals
- **Fast feedback** - Scenario switching is instant, no server restarts
...
// Regex pattern
match: { headers: { 'x-campaign': { regex: { source: 'premium|vip', flags: 'i' } } } }
```
