...
## 2. Create Instrumentation Hook (REQUIRED)

<Aside type="danger" title="⚠️ CRITICAL: This Step is REQUIRED">

**You MUST create this file for Scenarist to work with Next.js.** Without it, MSW won't be initialized before your getServerSideProps or API routes try to fetch data, causing real HTTP requests instead of mocked responses.
...
**Why this is required:** Next.js getServerSideProps and API routes run immediately on server startup. The instrumentation hook ensures MSW is initialized before any pages try to fetch data.
