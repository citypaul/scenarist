...
- ❌ **URL Field** (`url`) - Only exact string matching (no strategies)

:::note[URL Matching]
The `url` field only supports exact string matching. Use query parameter matching for dynamic URL values:
```typescript
// ❌ Cannot use strategies on URL field
url: { contains: '/api/users' }  // Not supported

// ✅ Use query parameter matching instead
url: '/api/users',
match: {
...
// Matches:
// /api/search?email=user@gmail.com ✓
// /api/search?email=user@YAHOO.com ✓ (case-insensitive)
// /api/search?email=user@hotmail.com ✗ (not in pattern)
```
...
// Matches:
// { filename: 'photo.jpg' } ✓
// { filename: 'avatar.PNG' } ✓ (case-insensitive)
// { filename: 'document.pdf' } ✗ (not an image)
```
