...
Other scenarios **override default by URL**. If a specialized scenario defines a mock for the same URL, it wins:
...
When mocks have **equal specificity**, the **first one wins**:
...
mocks: [
  // Both have specificity: 1 (body: type)
  {
    ...
    response: { status: 200, body: { discount: 20 } },  // ← This one wins
  },
  {
    match: { body: { type: 'premium' } },
...
**Best practice:** Avoid equal specificity. Make match criteria distinct.
