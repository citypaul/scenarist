...
Both Next.js adapters use the async pattern with **defense-in-depth** - two layers of production safety:
...
// Layer 1: Conditional exports (package.json)
// Layer 2: NODE_ENV check + dynamic imports
export const scenarist = await createScenarist({
  enabled: true,
  scenarios,
});
...
**Top-level await** is supported in Next.js 13+ and works seamlessly.

**Production behavior:**
- ✅ **Layer 1 (Build Time):** Conditional exports resolve to `production.js` which returns `undefined` immediately
- ✅ **Layer 2 (Runtime Fallback):** If Layer 1 fails, `NODE_ENV=production` check returns `undefined` before any dynamic imports execute
...
**Available helpers:**
- **App Router:** `getScenaristHeaders(request)`, `getScenaristHeadersFromReadonlyHeaders(headers)`, `SCENARIST_TEST_ID_HEADER`, `SCENARIST_DEFAULT_TEST_ID`
- **Pages Router:** `getScenaristHeaders(req)`

These helpers access global singletons and return empty objects in production, eliminating the need for manual undefined checks.
