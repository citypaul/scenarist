...
<Aside type="caution">
For teams bundling Express server code, **additional bundler configuration is required** to achieve complete tree-shaking.
</Aside>
...
**esbuild:**
```bash
esbuild --bundle --conditions=production --define:process.env.NODE_ENV='"production"'
```
...
# Should be ~298kb (not ~618kb)

# Search bundle for MSW code
...
```json
{
  "scripts": {
    ...
    "verify:treeshaking": "npm run build:production && ! grep -rE '(setupWorker|HttpResponse\\.json)' dist/"
  }
}
```
