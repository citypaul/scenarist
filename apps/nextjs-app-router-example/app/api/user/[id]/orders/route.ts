/**
 * Orders API Route - Abstracts database/external service access
 *
 * In a real application, this would query order history from database OR call an external service.
...
export async function GET(
  ...
  const response = await fetch(`http://localhost:3001/api/user/${params.id}/orders`, {
    headers,
  });
