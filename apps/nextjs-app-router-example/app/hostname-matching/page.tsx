...
export default async function HostnameMatchingPage({
  searchParams,
}: PageProps) {
  ...
  const fetchResult = await fetchTestData(
    testType,
    userId,
    postId,
    headersList,
      ...
      {fetchResult.success && (
        <div style={{ marginTop: "20px" }}>
          ...
          <div style={{ padding: "10px", background: "#e8f5e9" }}>
            <p>
              <strong>Pattern Type:</strong> {fetchResult.data.patternType}
            </p>
            {fetchResult.data.hostname && (
              <p>
                <strong>Hostname:</strong> {fetchResult.data.hostname}
              </p>
            )}
            <p>
              <strong>Behavior:</strong> {fetchResult.data.behavior}
            </p>
            <p>
              <strong>Message:</strong> {fetchResult.data.message}
            </p>

            {fetchResult.data.userId && (
              <p>
                <strong>User ID:</strong> {fetchResult.data.userId}
              </p>
            )}
            {fetchResult.data.postId && (
              <p>
            ...
            {fetchResult.data.willMatch && (
              <p>
                <strong>Will Match:</strong> {fetchResult.data.willMatch}
              </p>
            )}
