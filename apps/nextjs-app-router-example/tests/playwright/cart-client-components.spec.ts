...
test.describe("Cart - Client Components", () => {
  test("add product to cart shows item count", async ({
    page,
    switchScenario,
  }) => {
  ...
  test("add multiple products accumulates cart", async ({
    page,
    switchScenario,
  }) => {
    await switchScenario(page, "cartWithState");
  ...
  test("cart displays correct products and quantities", async ({
    page,
    switchScenario,
  }) => {
    await switchScenario(page, "cartWithState");
  ...
  test("cart persists across page navigation", async ({
    ...
    // Add items to cart (wait between to avoid GET-then-PATCH race)
    await addButtons.nth(0).click();
    await expect(cartCount).toHaveText("1");
    ...
    const cartListAgain = page.getByRole("list", {
