...
| **Scenario Management**   |                               |                               |                 |
| Scenario switching        | ✅ POST /**mock_scenario**    | ✅ POST /**scenario**         | ✅              |
...
export const addressFailure = createScenario(() => ({
  ...
  mocks: [
    http.post(
      `${CUSTOMER_API_URL}/api/address/get-address`,
      async ({ request }) => {
        const body = await request.json().catch(() => ({}));
        printInConsole(
          `MSW OVERRIDE handling: POST CUSTOMER/ADDRESS/GET-ADDRESS | body: ${JSON.stringify(body)}`,
        );
...
const loginVariants = {
  ...
  [OnlineJourneyLoginVariantKeys.downsell]: {
    ...
    description:
      "User offer has been downsell and has never gone through an openbanking check.",
    value: (): VariantResponse => ({
      state: UnifiedApiApplicationState.quoteAccept,
...
export const onlineJourneyLoginScenarios = createScenario((variant) => ({
  ...
  mocks: [
    http.get(`${UNIFIED_API_URL}/applications/:id`, async ({ request }) => {
      ...
      const { state, channel, subState, openBankingUserId } =
        loginVariants[parsedResult.name].value();

      // Complex conditional logic based on referer header
      const { remixHeadersParsed, remixParamsParsed } =
        await getRemixMetaInformation(request);
...
http.get(
  `${UNIFIED_API_URL}/products/:sourceCode/document/summary`,
  ({ params }) => {
